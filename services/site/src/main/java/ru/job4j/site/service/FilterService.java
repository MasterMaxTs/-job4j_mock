...
public class FilterService {
    ...
    public FilterDTO save(String token, FilterDTO filter) throws JsonProcessingException {
        restAuthCall.setUrl(URL);
        var mapper = new ObjectMapper();
        var out = restAuthCall.post(token, mapper.writeValueAsString(filter));
    ...
    public FilterDTO getByUserId(String token, int userId) throws JsonProcessingException {
        restAuthCall.setUrl(String.format("%s%d", URL, userId));
    ...
    public void deleteByUserId(String token, int userId) throws JsonProcessingException {
        var mapper = new ObjectMapper();
        restAuthCall.setUrl(String.format("%sdelete/%d", URL, userId));
        restAuthCall.delete(token, mapper.writeValueAsString(userId));
    }
}
